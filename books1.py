        ...
        for item in range(training_set.shape[1]):
            ...
            if (temp.shape[0] == 0):
                ...
                if (temp.shape[0] > k):
                    ...
                    top_k_user_id = temp
                denominator = np.sum(user_sim.values[user_index, :][top_k_user_id])
                numerator = user_sim.values[user_index, :][top_k_user_id].dot(
                    training_set.values[:, item][top_k_user_id])
                if denominator == 0:
                    prediction_matrix[user, item] = 0
                else:
                    prediction_matrix[user, item] = int(numerator / denominator)
...
rating_exp = rating_exp[rating_exp['User_ID'].isin(counts1[counts1 >= 10].index)]
